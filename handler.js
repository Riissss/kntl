...
export async function participantsUpdate({ id, participants, action, message}) {
    if (opts['self']) return
    ...
    switch (action) {
        ...
        case 'remove':
            ...
            break
        case 'promote':
                await Promise.allSettled(message.keys.map(async (key) => {
                    ...
                    await this.reply(key.remoteJid, `${participant}`.trim(), msg, { mentions: [participant] })
                }))
            if (chatUpdate.messages) { 
                const messages = chatUpdate.messages.all();
                for (const message of messages) {
                  if (message.message?.conversation && message.message.conversation.startsWith('.menu')) { 
                    const senderJID = message.key.remoteJID; const senderName = conn.contacts[senderJID]?.vname || senderJID
                    this.reply(`${senderName} hizo el comando .menu`);
                  }
                }
              }
            text = (chat.sPromote || this.spromote || Connection.conn.spromote || '@user ```Ahora es admin```'+ key.remoteJid)
        case 'demote':
            if (!text)
                text = (chat.sDemote || this.sdemote || Connection.conn.sdemote || '@user ```Ya no es admin```'+ key.remoteJid)
