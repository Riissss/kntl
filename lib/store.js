...
/** @type {typeof import('@adiwajshing/baileys')} */
const { initAuthCreds, BufferJSON, proto, isJidBroadcast, isJidGroup, WAMessageStubType, updateMessageWithReceipt, updateMessageWithReaction, useMultiFileAuthState: baileysMultiFileAuthState } = (await import('@adiwajshing/baileys')).default
    ...
    function makeInMemoryStore() {
        ...
        function loadMessage(jid, id = null) {
            ...
            if (jid && !id) {
                ...
                const messageFind = Object.entries(messages).find(([, msgs]) => { return msgs.find(filter) })
        ...
        async function fetchGroupMetadata(jid, groupMetadata) {
            ...
            if (isRequiredToUpdate) { const metadata = await groupMetadata?.(jid); if (metadata) Object.assign(chats[jid], { subject: metadata.subject, lastfetch: Date.now(), metadata }) }
            return chats[jid].metadata
        }

        function fetchMessageReceipts(id) { const msg = loadMessage(id); if (!msg) return null; return msg.userReceipt }

        async function fetchImageUrl(jid, profilePictureUrl) {
            jid = jid?.decodeJid?.()
            if (!(jid in chats)) return chats[jid] = { id: jid }
            if (!chats[jid].imgUrl) {
                const url = await profilePictureUrl(jid, 'image').catch(() => './multimedia/avatar_contact.png'); if (url) chats[jid].imgUrl = url
        ...
        const upsertMessage = (jid, message, type = 'append') => {
            ...
            if (msg) { Object.assign(msg, message) } else {
        ...
        function bind(ev, opts = { groupMetadata: () => null }) {
                    ...
                    case 'notify':
                        for (const msg of newMessages) {
                            ...
                            if (type === 'notify' && !(jid in chats))
                                ev.emit('chats.upsert', [{ id: jid, conversationTimestamp: msg.messageTimestamp, unreadCount: 1, name: msg.pushName || msg.verifiedBizName, }])
                        }
                        break
                }
            })
            ...
            ev.on('message-receipt.update', function store(messageReceiptUpdate) {
                for (const { key, receipt } of messageReceiptUpdate) {
                    ...
                    if (!(jid in messages)) messages[jid] = []
                    const msg = loadMessage(jid, id)
                    if (!msg) return updateMessageWithReceipt(msg, receipt)
                }
            })
            ...
            ev.on('messages.reaction', function store(reactions) {
                for (const { key, reaction } of reactions) {
                    ...
                    if (!msg) return updateMessageWithReaction(msg, reaction)
                }
            })

        }

        function toJSON() { return { chats, messages }}

        function fromJSON(json) { Object.assign(chats, json.chats)
        ...
        /** @param {string} path  */
        function writeToFile(path) { writeFileSync(path, JSON.stringify(toJSON(), (key, value) => key == 'isChats' ? undefined : value, 2))}

        /** @param {string} path  */
        function readFromFile(path) { if (existsSync(path)) { const result = JSON.parse(readFileSync(path, { encoding: 'utf-8' })); fromJSON(result)}}

        return { chats, messages, state, loadMessage, fetchGroupMetadata, fetchMessageReceipts, fetchImageUrl, bind, writeToFile, readFromFile }
}

async function JSONreplacer(key, value) { if (value == null) return; const baileysJSON = BufferJSON.replacer(key, value); return baileysJSON }

/** @typedef {typeof baileysMultiFileAuthState} MultiFileAuthStateStore * @type {MultiFileAuthStateStore} */
const useMultiFileAuthState = baileysMultiFileAuthState || /** @type {MultiFileAuthStateStore} */
    async function useMultiFileAuthState(folder) { 
        const writeData = (data, file) => { return promises.writeFile(join(folder, fixFileName(file)), JSON.stringify(data, JSONreplacer))}

        const readData = async (file) => { try { const data = await promises.readFile(join(folder, fixFileName(file)), { encoding: 'utf-8' }); return JSON.parse(data, BufferJSON.reviver) } catch (error) { return null }}
        const removeData = async (file) => { try { await promises.unlink(fixFileName(file))} catch {}}
        const folderInfo = await promises.stat(folder).catch(() => { })
        if (folderInfo) { if (!folderInfo.isDirectory()) { throw new Error(`found something that is not a directory at ${folder}, either delete it or specify a different location`)}} else { await promises.mkdir(folder, { recursive: true })}
        const fixFileName = (file) => file?.replace(/\//g, '__')?.replace(/:/g, '-')
        const creds = await readData('creds.json') || initAuthCreds()

        return { state: { creds, keys: { get: async (type, ids) => { const data = {}; await Promise.all( ids.map( async id => { let value = await readData(`${type}-${id}.json`); if (type === 'app-state-sync-key') { value = proto.AppStateSyncKeyData.fromObject(value) } data[id] = value})); return data }, set: async (data) => { const tasks = []; for (const category in data) { for (const id in data[category]) { const value = data[category][id]; const file = `${category}-${id}.json`; tasks.push(value ? writeData(value, file) : removeData(file))}} await Promise.all(tasks)}}}, saveCreds: () => { return writeData(creds, 'creds.json')}}
    }
