...
const { DisconnectReason, Browsers, default: makeWASocket,  fetchLatestBaileysVersion } = (await import("@adiwajshing/baileys")).default

const authFolder = 'VarySesion'
const storeFile = `${Helper.opts._[0] || 'datos'}.store.json`
const authState = await storeSystem.useMultiFileAuthState(authFolder)
const store = storeSystem.makeInMemoryStore()
let { version } = await fetchLatestBaileysVersion()
const connectionOptions = { logger: pino({ level: 'silent' }), browser: Browsers.macOS('Desktop'), printQRInTerminal: true, auth: authState.state, version }
store.readFromFile(storeFile)

let conns = new Map();
async function start(oldSocket = null, opts = { store }) { let conn = makeWASocket({ ...connectionOptions, ...opts.connectionOptions, getMessage: async (key) => (opts.store.loadMessage((key.remoteJid), key.id) || opts.store.loadMessage((key.id)) || {}).message || { conversation: 'Por favor, envíe mensajes de nuevo' }, }); global.teslagod = conn; HelperConnection(conn); if (oldSocket) { conn.isInit = oldSocket.isInit; conn.isReloadInit = oldSocket.isReloadInit }; if (conn.isInit == null) { conn.isInit = false; conn.isReloadInit = true }; store.bind(conn.ev, { groupMetadata: conn.groupMetadata }); await reload(conn, false, opts).then((success) => console.log(chalk.bgWhite('Evento controlador de enlace : ', success + '\n'))); return conn }
...
async function reload(conn, restartConnection, opts = {}) {
    ...
    if (restartConnection) { try { conn.ws.close() } catch { }; conn.ev.removeAllListeners(); Object.assign(conn, await start(conn) || {}) } Object.assign(conn, getMessageConfig())
...
export default { start, reload, conn, conns, connectionOptions, authFolder, storeFile, authState, store, getMessageConfig }
