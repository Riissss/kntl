...
let handler = async (m, { conn, text }) => {
    let chat = global.db.data.chats[m.chat].cmdDl && m.isGroup
	if (!chat) return m.reply(`_Este comando está deshabilitado para este grupo._`)
    ...
    await conn.sendMessage(m.chat, { document: { url: videoUrl }, caption: cap, mimetype: 'video/mp4', fileName: title + `.mp4` }, { quoted: m }); m.react(done); if (global.ajustes.cmdRpg) return m.coin = true } catch { await m.react(error) }
}
